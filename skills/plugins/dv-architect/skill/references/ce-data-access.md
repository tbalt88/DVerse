...
> Ported into DVerse v2 from the archived seed `d365-architect` skill. Table
> and column examples below use the `dv_` schema prefix, this repo's actual
> prefix (mechanically enforced by G2), not the seed's `dexx_` placeholder.
> See `../SKILL.md`, "v2 reality".
...
<fetch version="1.0" output-format="xml-platform" mapping="logical"
  ...
  <entity name="dv_hcpvisit">
    <attribute name="dv_hcpvisitid" />
    <attribute name="dv_visitdate" />
    <attribute name="dv_status" />
    <link-entity name="contact" from="contactid" to="dv_hcpid"
                 link-type="inner" alias="hcp">
      <attribute name="fullname" />
      <attribute name="emailaddress1" />
    </link-entity>
    <filter type="and">
      <condition attribute="dv_status" operator="eq" value="1" />
      <condition attribute="dv_visitdate" operator="on-or-after"
                 value="2025-01-01" />
    </filter>
    <order attribute="dv_visitdate" descending="true" />
  </entity>
</fetch>
...
<fetch aggregate="true">
  <entity name="dv_hcpvisit">
    <attribute name="dv_hcpvisitid" aggregate="count" alias="visit_count" />
    <attribute name="dv_hcpid" groupby="true" alias="hcp_id" />
    <filter>
...
### Execute FetchXML via Web API (Bash tool)
```bash
...
curl -s "$ENV_URL/api/data/v9.2/dv_hcpvisits?\$fetchXml=$ENCODED" \
...
function onFormLoad(executionContext) {
    ...
    // Set field value
    formContext.getAttribute("dv_priority").setValue(2);

    // Show/hide control
    formContext.getControl("dv_creditlimit").setVisible(status === 1);

    // Lock field
    formContext.getControl("dv_orderdate").setDisabled(true);
...
async function fetchRelatedRecords(primaryId) {
    const result = await Xrm.WebApi.retrieveMultipleRecords(
        "dv_hcpvisit",
        `?$select=dv_visitdate,dv_status&$filter=_dv_hcpid_value eq ${primaryId}&$top=10`
    );
...
|---|---|---|
| Table schema name | `prefix_entityname` | `dv_hcpvisit` |
| Column schema name | `prefix_columnname` | `dv_visitdate` |
| Publisher prefix | 2–8 lowercase letters | `dv` |
| Plugin class | `Namespace.Plugins.OnVerbEntity` | `Contoso.Plugins.OnCreateOrder` |
| Web resource | `prefix_/folder/file.ext` | `dv_/js/orderform.js` |
