...
> Ported into DVerse v2 from the archived seed `d365-architect` skill. The
> virtual entity example below uses the `dv_` schema prefix, this repo's
> actual prefix (mechanically enforced by G2), not the seed's `dexx_`
> placeholder. See `../SKILL.md`, "v2 reality".
...
public class VirtualHcpDataProvider : IPlugin
{
    public void Execute(IServiceProvider serviceProvider)
    {
        ...
        {
            var entity = new Entity("dv_externalhcp");
            entity["dv_externalhcpid"] = item.Id;
            entity["dv_name"] = item.Name;
