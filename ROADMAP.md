...
| O7 | **Normalise `Artifact` path base across gates.** G2 and G4 emit relative to `SolutionRoot`, G9 and G10 relative to `RepositoryRoot`. Both satisfied their manifests, but the ledger is the receipt and a reader cannot tell what a path is relative to. Found by live CLI run, not by tests. Fix the contract to state one base and add a gate-agnostic assertion. | wave 2.1 |
